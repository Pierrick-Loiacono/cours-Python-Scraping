# Choix d'un sujet

Par groupe de 2 ou 3, vous choisissez un sujet que vous développerez. Vous devrez préparer une présentation ainsi qu'une démo fonctionnelle

## Sujet 1 - 

Voir le fichier [Sujet1.pdf](./fichiers/Sujet1.pdf)

## Sujet 2 - 
Voir le fichier [Sujet2.pdf](./fichiers/Sujet2.pdf)

## Sujet 3 - Projet de Monitoring

Voir le fichier [Sujet3.pdf](./fichiers/Sujet3.pdf)

## Autre

Vous êtes libre de proposer un sujet de votre choix, pensez à m'en parler pour que je le valide

## Oral

Chaque groupe devra présenter son projet avec démo, d'une durée total de 15 minutes

Vous présenterez :
- L'objectif de votre application
- Comment fonctionne votre projet
- La structure des données
- La structure de votre projet
- Difficultés rencontrées
- Une démonstration

> Vous êtes libres de vous organiser comme vous le souhaitez, ceci n'est qu'une suggestion.